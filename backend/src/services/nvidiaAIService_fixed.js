...
class NvidiaAIService {
    ...
    async chatWithAI(userMessage, context = '') {
        ...
        for (let index = 0; index < CHAT_MODELS.length; index++) {
            ...
            try {
                ...
                if (!content) {
                    throw new Error('NVIDIA API returned an empty AI response.');
                }
                ...
                lastError = error;
                const msg = this._extractError(error, model);

                if (this._isModelFailure(error) && index < CHAT_MODELS.length - 1) {
                    console.warn(
                        `⚠️ NVIDIA model unavailable: ${model}. Falling back to: ${CHAT_MODELS[index + 1]}`
                    );
                    continue;
                }

                console.error(`❌ NVIDIA chat error (${model}):`, msg);
                break;
            }
        }

        const msg = this._extractError(lastError, CHAT_MODELS[CHAT_MODELS.length - 1]);
        console.error('❌ All NVIDIA chat models failed:', msg);

        return {
            success: false,
            error: `AI temporarily unavailable. Tried ${CHAT_MODELS.length} model(s). Last error: ${msg}`
        };
    }
    ...
    async generateVideoFromImage(imageUrl, prompt, options = {}) {
        ...
        try {
            ...
            const msg = this._extractError(error, VIDEO_MODEL);
