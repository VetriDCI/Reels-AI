...
export default function SavedPostsPage({ onBack, searchQuery = '' }) {
  ...
  const load = async () => {
    ...
    catch (e) { console.error('Saved posts load error', e); setError(e.response?.data?.message || 'Unable to load saved posts.'); }
  ...
  const remove = async (id) => { if (!window.confirm('Remove this saved item?')) return; try { setError(''); await savedPostAPI.remove(id); setItems(prev => prev.filter(x => x.post?.id !== id)); } catch (e) { setError(e.response?.data?.message || 'Unable to remove saved post.'); } };
