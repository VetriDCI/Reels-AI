...
import { payoutAPI, monetizationAPI } from '../../services/api';

export default function BillingEarningsPage({ user, onBack }) {
  const [earnings, setEarnings] = useState(Number(user?.earnings || 0));
  ...
  const load = async (nextPage = 1) => {
    try {
      setError('');
      const [payoutRes, analyticsRes] = await Promise.all([payoutAPI.list(nextPage, 20), monetizationAPI.analytics(30)]);
      setPayouts(nextPage === 1 ? (payoutRes.data?.data || []) : [...payouts, ...(payoutRes.data?.data || [])]);
      setHasMore(Boolean(payoutRes.data?.pagination?.hasMore));
      setPage(nextPage);
      setEarnings(Number(analyticsRes.data?.data?.lifetimeEarnings ?? user?.earnings ?? 0));
    } catch (e) { setError(e.response?.data?.message || 'Unable to load earnings and payout history.'); } finally { setLoading(false); }
  };
  ...
  const submit = async (e) => {
    ...
    try { setSubmitting(true); await payoutAPI.request({ amount:n, method, accountLabel:accountLabel.trim() }); setAmount(''); setAccountLabel(''); setMessage('Payout request submitted.'); await load(1); }
  ...
  return <div className="min-h-screen bg-gradient-to-br from-pink-100 to-blue-100">
    ...
    <div className="p-4">
      {error && <div className="mb-4 rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-700">{error} <button onClick={() => load(1)} className="ml-2 font-semibold underline">Retry</button></div>}
