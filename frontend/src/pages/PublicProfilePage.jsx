...
export default function PublicProfilePage({ userId, onBack }) {
  ...
  const load = async () => {
    ...
    } catch (e) { setError(e.response?.data?.message || 'Unable to load profile'); } finally { setLoading(false); }
  };
  ...
  const toggleFollow = async () => {
    ...
    catch (e) { setError(e.response?.data?.message || 'Unable to update follow'); }
    finally { setFollowLoading(false); }
  };

  if (loading) return <div className="min-h-screen flex items-center justify-center text-gray-500">Loading profile...</div>;
  if (!profile) return <div className="p-6"><button onClick={onBack} aria-label="Back"><ArrowLeft /></button>{error ? <p className="mt-6 text-red-600">{error}</p> : <p className="mt-6">Profile not found.</p>}</div>;
